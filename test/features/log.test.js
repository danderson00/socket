...
test("log entries from consumer are logged to both consumer and host log streams", async () => {
  ...
  expect(consumerWrite.mock.calls[0][0]).toEqual({ level: 2, p: 1, source: 'consumer' })
  expect(consumerWrite.mock.calls[1][0]).toMatchObject({ level: 1, message: 'test error', source: 'consumer' })
  expect(hostWrite.mock.calls.length).toBe(2)
  expect(hostWrite.mock.calls[0][0]).toEqual({ level: 2, p: 1, source: 'consumer' })
  expect(hostWrite.mock.calls[1][0]).toMatchObject({ level: 1, message: 'test error', source: 'consumer' })
})